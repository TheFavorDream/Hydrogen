...
namespace Hydrogen
{
	...
	GLVertexArray& GLVertexArray::operator=(GLVertexArray&& pOther)
	{
		if (this != &pOther)
		{
	...
	uint32 GLVertexArray::CreateVertexArray()
	{
		GL_CALL(glGenVertexArrays(1, &m_VertexID));
		return HYD_OK;
	}

	uint32 GLVertexArray::DestroyVertexArray()
	{
		Unbind();
	...
	uint32 GLVertexArray::AddAttribute(const Layout& pLayout)
	{	
	...
	uint32 GLVertexArray::AddAttributes(const std::vector<Layout>& pAttributes)
	{
	...
	uint32 GLVertexArray::EnableAttribute(uint32 pAttribute)
	{
	...
	uint32 GLVertexArray::DisableAttributes(uint32 pAttribute)
	{
		if (pAttribute >= m_Attributes)
			return HYD_INVALID_VALUE;
	...
	GLenum GLVertexArray::GetGLType(DataType pType)
	{
		switch (pType)
		{
		case TYPE_FLOAT:
	...
	DataType GLVertexArray::GetHydType(GLenum pType)
	{
		switch (pType)
		{
		case GL_FLOAT:
	...
	uint32 GLVertexArray::GetTypeSize(DataType pType)
	{
